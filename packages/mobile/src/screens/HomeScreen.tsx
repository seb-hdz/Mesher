...
export function HomeScreen({ navigation }: HomeScreenProps) {
  ...
  const renderNearbyCount = () => {
    return ready && !bluetoothUnavailable ? (
      <Alert
        type={neighborCount > 0 ? "info" : "error"}
        content={
          <>
            <Text
              className={cn(
            ...
            <Text className="font-semibold">{l("HOME.NEARBY_CAPTION")}</Text>
          </>
        }
      />
    ) : null;
  ...
  return (
    <ScreenContainer className="flex-1 bg-background px-0">
      ...
      <View className="flex-1 justify-between px-4 pt-4">
        <View className="max-w-[250px] z-10">
        ...
        >
          ...
          <Button
            ...
            disabled={!ready || bluetoothUnavailable}
          >
