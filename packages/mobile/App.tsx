...
function readScheduledJobId(data: Record<string, unknown> | undefined): string | null {
  if (!data) return null;
  const type = data.type;
  const jobId = data.jobId;
  if (type === "scheduled_send" && typeof jobId === "string" && jobId.length > 0) {
    return jobId;
  }
  return null;
}
  ...
  useEffect(() => {
    ...
    const processJob = (jobId: string) => {
      ...
      if (!store.ready) {
        ...
        console.log(`[mesher:siri] deferred scheduled job until ready jobId=${jobId}`);
        return;
      }
